...
const TABS = [
  { id: '1', label: 'เพิ่มชิ้นงานเข้าระบบรายชิ้น', desc: 'เพิ่มชิ้นงานทีละชิ้นพร้อมรายละเอียดและไฟล์แบบ' },
  { id: '2', label: 'เพิ่มชิ้นงานเข้าระบบด้วย Excel', desc: 'นำเข้าชิ้นงานจำนวนมากพร้อมกันจากไฟล์ Excel' },
  { id: '3', label: 'จัดการชิ้นงานอื่นๆ', desc: 'แก้ไขและลบชิ้นงานประเภทอื่นที่มีอยู่ในระบบ' },
  { id: '4', label: 'จัดการชิ้นงานพรีคาสท์', desc: 'แก้ไขและลบชิ้นงานพรีคาสท์ที่มีอยู่ในระบบ' },
];
  ...
  return (
    <PageContainer title="จัดการชิ้นงานของแต่ละโครงการ" description="Manage components individually or in bulk">
      <div className="mes-card">
        ...
        <div className="flex gap-1 snap-x overflow-x-auto border-b border-mes-border px-3 pt-2">
          {TABS.map((t) => (
            <button
              key={t.id}
              onClick={() => setTab(t.id)}
              className={`min-h-touch md:min-h-0 shrink-0 snap-start whitespace-nowrap rounded-t-sm px-3 py-2 text-sm font-semibold border-b-2 -mb-px ${
        ...
        <p className="px-4 pt-3 text-sm text-mes-muted">{TABS.find((t) => t.id === tab)?.desc}</p>
