    ...
    Promise.all([import('gsap'), import('gsap/ScrollTrigger')])
      .then(([gsapModule, stModule]) => {
        ...
        ctx = gsap.context(() => {
          ...
          if (video) {
            // 0.45: the bespoke footage is already navy-graded with crushed blacks,
            // so it carries far more opacity than a bright generic loop would.
            gsap.to(video, { opacity: 0.45, duration: 1.8, ease: 'power2.inOut' });
            gsap.to(video, {
              scale: 1.08,
              ease: 'none',
