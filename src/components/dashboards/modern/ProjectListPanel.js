...
const ProjectRow = memo(
  ({ project, index, userRole, selectedProjectId, onProjectSelect, isSmallScreen, userProjectIds }) => {
    const [modalOpen, setModalOpen] = useState(false);
    const [loadingProgress, setLoadingProgress] = useState(0);
    const [localProject, setLocalProject] = useState(project);
    const theme = useTheme();

    // In ProjectListPanel.js, update the hasProjectPermission check
    ...
    return (
      <>
          ...
          sx={{
            backgroundColor:
              project.id === selectedProjectId
                ...
                ? alpha('#64b5f6', 0.08)
                : 'inherit',
            borderBottom: `1px solid ${alpha('#64b5f6', 0.25)}`,
            '& td': { borderBottom: 0 },
            '&:hover': {
        ...
        >
          ...
          <StyledDataCell>{project.project_code}</StyledDataCell>
          {!isSmallScreen && <StyledDataCell>{project.name}</StyledDataCell>}
          {!isSmallScreen && <StyledDataCell align="right">{numberOfSections}</StyledDataCell>}
          <StyledDataCell align="right">{totalComponents}</StyledDataCell>
        </TableRow>
...
// Main ProjectListPanel Component
const ProjectListPanel = memo(({ onProjectSelect, userRole, refreshTrigger, onTabChange, userId }) => {
  const theme = useTheme();
  ...
  return (
    <Paper
      elevation={3}
      sx={{
        p: isSmallScreen ? 1 : { xs: 1, sm: 2, md: 1.5 },
